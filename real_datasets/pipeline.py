...
# Import our custom modules for real data
from data_loader import load_real_data, attn_f, feat_cols, C_FP, C_FN_m
...
def run_full(seed=42, sample_size=50000):
    """
    Run a complete experimental training and evaluation loop on the real dataset for a single seed.
    """
    df = load_real_data(sample_size=sample_size, seed=seed)
...
def run_pipeline(force_rerun=False, sample_size=50000):
    """
    Run the experimental pipeline on the real dataset and cache the results.
    """
    cache_path = os.path.join(CACHE_DIR, 'pipeline_state.pkl')
    
    if os.path.exists(cache_path) and not force_rerun:
        print("Pipeline state cache for real dataset found! Loading cached results.")
        with open(cache_path, 'rb') as f:
            return pickle.load(f)
            
    print("="*55)
    print("  Running Real Credit Card Dataset Pipeline")
    print("="*55)
    ...
    for sd in seeds:
        ...
        r = run_full(sd, sample_size=sample_size)
        runs.append(r)
        m = r[0]
    ...
    print("\n  Cross-seed results on Real Dataset:")
    for nm in mn:
        s = stats[nm]
    ...
    # ═══════════════════════════════════════
    print("\n  Running Novel Fraud Experiment on Real Dataset...")
    df_nf = load_real_data(sample_size=sample_size, seed=42)
    fraud_m = df_nf.Class == 1
    
    # Partition fraud dynamically based on V3 quantile if it's too skewed, or standard -1 threshold
    # Since V3 might have different boundaries in real dataset, let's use the 40th percentile of V3 of fraud
    # cases as the boundary to split into known and novel fraud.
    fraud_v3 = df_nf[fraud_m].V3.values
    if len(fraud_v3) > 5:
        split_val = np.percentile(fraud_v3, 40)
    else:
        split_val = -1.0
        
    known = fraud_m & (df_nf.V3 < split_val)
    novel = fraud_m & (df_nf.V3 >= split_val)
    train_m = ~novel
    ...
    # Train models on known fraud and legitimate samples
    ...
    # Dynamic detection rates based on relative model thresholds
    t_rf, _ = bf1(y_te, md0['RF'])
    t_xgb, _ = bf1(y_te, md0['XGBoost'])
    t_nn, _ = bf1(y_te, md0['AADNN'])
    
    nfr = {
        'RF': np.mean(rf_nfp >= t_rf),
        'XGBoost': np.mean(xgb_nfp >= t_xgb),
        'AADNN': np.mean(nn_nfp >= t_nn),
    ...
    # ═══════════════════════════════════════
    print("  Running Noise Robustness Experiment on Real Dataset...")
    nls = [0, 5, 10, 15, 20]
    rob = {nm: [] for nm in mn}
    for nl in nls:
        np.random.seed(42)
    ...
    # ═══════════════════════════════════════
    print("  Running Cost Sensitivity Experiment on Real Dataset...")
    ...
    # SHAP ANALYSIS
    # ═══════════════════════════════════════
    print("  Running SHAP Feature Attribution on Real Dataset...")
